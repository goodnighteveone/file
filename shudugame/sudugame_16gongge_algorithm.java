...
public class sudugame_16gongge_algorithm {
...
//                    "000G8D3C090AF041";
//
//    private final String answer = "BF423C8E1D6G957A" +
//            "A815DG27BCF9436E" +
//            "D63E915A4278CBFG" +
//            "9G7CB64FA35E2D18" +
//            "F261AEC97B843G5D" +
...
//            "67BG8D3C592AFE41";
     ...
     */
    //暂时存储数独初始数据
    protected static int[] temp_sudoku = new int[16 * 16];
    //数独初始数据的存储
    ...
    //用于存储每个单元格已经不可用的数据
    public int used[][][] = new int[16][16][];
    //数独游戏生成器
    sudu_16gongge_algorithm_shape sdas = new sudu_16gongge_algorithm_shape();

    //构造方法
    public sudugame_16gongge_algorithm(int grade) {

        int temp[][] = sdas.generate(grade);

        for (int i = 0; i < 16; i++)
            for (int j = 0; j < 16; j++) {
                sudoku2[i * 16 + j] = temp[i][j];
                sudoku[i * 16 + j] = temp[i][j];
            }
...
//        }
    ...
    public String getTileString(int x, int y, int[] sudo) {
        ...
        if (v == 0) {
            ...
            return String.valueOf((char) (v + 55));
        }
        //如果不是0，绘制其本身数字
        else
